...
class LotteryAdmin(admin.ModelAdmin):
    list_display = ("id", "total_tickets", "remaining_tickets", "is_active", "created_at", "updated_at")
...
@admin.register(models.LotteryParticipant)
class LotteryParticipantAdmin(admin.ModelAdmin):
    list_display = ("id", "username", "wallet_address", "tickets_count", "created_at", "updated_at")
    list_filter = ("created_at", "tickets_count")
    ...
    fieldsets = (
        ("Информация об участнике", {
            "fields": ("user", "username", "wallet_address")
        }),
        ("Лотерея", {
            "fields": ("tickets_count", "transaction_hash")
        }),
        ("Временные метки", {
            "fields": ("created_at", "updated_at"),
            "classes": ("collapse",)
