...
**Перегрев наступает по цели накопленной генерации (overheat_goal), а не при достижении storage = storage_limit.**
...
- **Триггер:** в процессе генерации энергии при достижении **цели по накопленной генерации** (`overheat_energy_collected >= overheat_goal`), а не при storage = storage_limit. Каждую минуту к storage и к overheat_energy_collected прибавляется одна и та же величина; первый перегрев — по случайной цели, следующий — по фиксированной (generation_rate × needed_hours). Тип станции из `OVERHEAT_HOURS_BY_TYPE`, без криокамеры. При срабатывании выставляется `overheated_until`, отправляется сообщение в бот.
...
## 8. Как тестировать перегрев

Перегрев срабатывает в **generation.py** при достижении **overheat_energy_collected >= overheat_goal** (цель по накопленной генерации), а не при storage = storage_limit.

### В админке Django

1. **OverheatConfig** (если есть в меню):
   - Задать **min_duration** и **max_duration** в секундах (например, 30 и 60) для быстрого теста.
   - Если записей нет — скрипт использует 30 и 300 по умолчанию.

2. **UserProfile** — тестовый пользователь для срабатывания перегрева на следующем тике:
   - **station_type** — один из: `Thermal power plant`, `Geothermal power plant`, `Nuclear power plant`, `Thermonuclear power plant`, `Dyson Sphere` (или Neutron star / Antimatter / Galactic core).
   - **overheated_until** — пусто (null).
   - **overheat_goal** — задать вручную, например **1** (кВт).
   - **overheat_energy_collected** — задать **>= overheat_goal**, например **1** или **2** (кВт), чтобы на следующем тике сразу сработал перегрев.
   - **cryo_expires** — пусто или в прошлом.
   - **user_id** — Telegram ID (число) для сообщения в бот.
   - Сохранить.

Либо для проверки «первого» перегрева: **overheat_goal** оставить пустым (null), **overheat_energy_collected** = 0; на первом тике скрипт выставит случайный goal, дальше при достижении цели сработает перегрев.

### Запуск генерации

- Запустить скрипт генерации (например, `python edit/generation.py` или крон).
- В течение следующего тика (~1 мин) скрипт:
  - прибавит к storage и к overheat_energy_collected одну и ту же величину (generation_rate × power/100 / 60),
  - для пользователей без goal выставит случайный **overheat_goal**,
  - при **overheat_energy_collected >= overheat_goal** выставит **overheated_until**, **was_overheated=True** и отправит сообщение в Telegram.

### Проверка в приложении

- Залогиниться под этим пользователем и открыть экран генерации (Energizer):
  - Должен отображаться блок перегрева, кнопка «Собрать энергию» скрыта.
- Подождать окончания охлаждения (**overheated_until** в прошлом):
  - Должны появиться AfterHeatModal и HeatSwitch (кнопка включения станции).
- Нажать включение станции (отправляется `enable-station/`):
  - Перегрев должен сброситься, кнопка «Собрать энергию» снова доступна.

### Быстрый ручной тест без ожидания генерации

Если не хочется ждать тик генерации, можно в админке вручную выставить **overheated_until** на ближайшее будущее (например, now + 1 минута) и сохранить. Тогда в приложении сразу будет виден активный перегрев (блок перегрева, скрытая кнопка «Собрать энергию»), а через минуту — сценарий «охлаждение закончилось» → AfterHeatModal → HeatSwitch → enable-station.
