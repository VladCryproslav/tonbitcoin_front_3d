...
Аналогично Jarvis, но применяется в цикле генерации (каждую минуту):

```
power_reduction_per_minute = (1 / 3600 * sbt_get_power()) * 60
```

Или упрощенно:
```
power_reduction_per_minute = (1 / 60 * sbt_get_power())
```

**Компоненты:**
- `1 / 3600` - снижение на 1 единицу за час (3600 секунд)
- `* 60` - преобразование в снижение за минуту (т.к. цикл каждую минуту)
...
**Примеры снижения:**
- Без бонусов: `1/60 * 1.0 = 0.0167% в минуту` или `1% в час` или `24% в сутки`
- С Premium/Gold SBT: `1/60 * 0.9 = 0.015% в минуту` или `0.9% в час` или `21.6% в сутки`
- С Silver SBT: `1/60 * 0.95 = 0.0158% в минуту` или `0.95% в час` или `22.8% в сутки`
...
**По умолчанию** (`POWER_REDUCTION_ONLY_WHEN_STORAGE_NOT_FULL = False`):
- **Без бонусов:** Power снижается на `24% в сутки` постоянно
- **С Premium/Gold SBT:** Power снижается на `21.6% в сутки` постоянно
- **С Silver SBT:** Power снижается на `22.8% в сутки` постоянно
