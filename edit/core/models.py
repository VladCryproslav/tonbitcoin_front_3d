...
class RunnerConfig(models.Model):
    ...
    # Максимальный "возраст" аккаунта в днях, в течение которого действует льготный станционный бонус.
    # Если пользователь зарегистрирован дольше, чем на этот срок, бонус по уровням станции 1–3 не применяется
    # и используется обычная логика, как для станций 4–10.
    lose_bonus_newbie_days = models.IntegerField(
        default=30,
        help_text="Сколько дней с момента регистрации считается \"новичком\" для применения станционного бонуса (0 или меньше — без ограничения по дате регистрации)"
    )
