...
async def main():
    ...
    connector = TonConnect(manifest_url="https://tbtc.one/tonconnect-manifest.json")

    def status_changed(wallet_info: WalletInfo):
        print("wallet_info:", wallet_info)
        print(proof_payload)
        print(wallet_info.ton_proof.payload)
    ...
    generated_url = await connector.connect(
        wallets_list[0], {"ton_proof": proof_payload}
    )
    ...
    for i in range(120):
        ...
        print(
            TonConnect(manifest_url="https://tbtc.one/tonconnect-manifest.json").wallet
        )
