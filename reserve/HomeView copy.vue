...
async function fetchChart() {
  ...
  try {
    ...
    if (!app?.dedust_kw) {
      promises.push(axios.post('https://api.dedust.io/v3/graphql', {
        query: `
  ...
  `,
    ...
    if (!app?.stonfi_fbtc) {
      promises.push(host.get('gecko/v2/networks/ton/pools/EQDRJ6wZJeaYYcR3FrqaShDgV2SyDtKBwoGI_wChiTrXL9mr/', { signal, timeout: 5000 }));
    }
    ...
    // --- Запити, що залежать від підключеного гаманця ---
    if (connectedAddressString.value) {
      promises.push(tonapi.get(`accounts/${connectedAddressString.value}/nfts?collection=0:c9511472ee373f1aeb5d2dd12fc5f5cbf43d30cc1c9f0e23ad2f00346ea9e205`, { signal }));
      promises.push(tonapi.get(`accounts/${connectedAddressString.value}/nfts?collection=0:3126dc7f3db3b6901eb79de8d54a308d7ded51396d04d7ebcb218c48b536b25b`, { signal }));
      promises.push(tonapi.get(`accounts/${connectedAddressString.value}`, { signal }));
      promises.push(tonapi.get(`accounts/${connectedAddressString.value}/jettons`, { signal }));
    }

    // --- Запити, які можна кешувати ---
    if (!app?.rentedNfts.length) promises.push(host.get("user-rented-nfts/", { signal }));
    if (!app?.rentOutNfts.length) promises.push(host.get("user-lent-nfts/", { signal }));

    // Виконуємо всі базові запити паралельно
    await Promise.allSettled(promises);
    // Тут можна обробити результати першої пачки запитів, щоб UI оновився ще швидше
    // --- ОПТИМІЗАЦІЯ: Паралельне завантаження всіх NFT, якщо їх немає в сторі ---
    if (!app.all_asics.length) {
      ...
      // 1. Отримуємо загальну кількість NFT в колекціях, щоб знати кількість сторінок
      const [col1Info, col2Info] = await Promise.all([
        tonapi.get(`nfts/collections/${collection1Addr}`, { signal }),
        tonapi.get(`nfts/collections/${collection2Addr}`, { signal })
      ]);

      const nfts1_length = Math.ceil(col1Info.data?.next_item_index / 1000);
      const nfts2_length = Math.ceil(col2Info.data?.next_item_index / 1000);

      const allPagesPromises = [];
      // 2. Створюємо масив промісів для кожної сторінки
      for (let i = 0; i < nfts1_length; i++) {
        allPagesPromises.push(tonapi.get(`nfts/collections/${collection1Addr}/items?offset=${i * 1000}`, { signal }));
      }
      for (let i = 0; i < nfts2_length; i++) {
        allPagesPromises.push(tonapi.get(`nfts/collections/${collection2Addr}/items?offset=${i * 1000}`, { signal }));
      }

      // 3. Запускаємо завантаження всіх сторінок паралельно
      const allPagesResults = await Promise.allSettled(allPagesPromises);

      // 4. Збираємо результати в один масив
      const allNfts = allPagesResults
        .filter(res => res.status === 'fulfilled' && res.value.status === 200)
        .flatMap(res => res.value.data?.nft_items);

      app.setAllAsics(allNfts);
    }
  } catch (err) {
    console.log(err)
  } finally {
