  ...
  <div class="modal-mask" name="modal">
    <div class="modal-wrapper">
      <div class="modal-container">
        ...
        <div class="grouping">
          <div class="modal-header">{{ t('modals.withdraw_rent_modal.attention') }}</div>
          <div class="modal-body">
            {{ t('modals.withdraw_rent_modal.confirm_withdraw_message', { amount: +props.sum.toFixed(2) }) }}
          </div>
          <div class="buttons-group">
            <button class="confirm" @click="withdrawTBTC">{{ t('modals.withdraw_rent_modal.confirm') }}</button>
            <button class="cancel" @click="emitClose">{{ t('modals.withdraw_rent_modal.cancel') }}</button>
          </div>
        </div>
      </div>
    </div>
      ...
      .then((res) => {
        if (res.status == 200) {
          emit('close', {
            ...
            body: t('modals.withdraw_rent_modal.withdraw_request_accepted', { amount: props.sum }),
          })
        }
      })
      .catch((err) => {
        console.error(err)
        emit('close', { status: 'error', title: t('notification.st_error'), body: err.response.data.error == 'You can only make one withdrawal request per day' ? t('modals.withdraw_rent_modal.withdraw_once_per_day') : err.response.data.error })
      })
  } catch (err) {
    emit('close', { status: 'error', title: t('notification.st_error'), body: err.response.data.error == 'You can only make one withdrawal request per day' ? t('modals.withdraw_rent_modal.withdraw_once_per_day') : err.response.data.error })
  }
