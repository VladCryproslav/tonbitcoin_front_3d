<script setup>
import asicsSheet from '@/services/data';
import confetti from 'canvas-confetti';
import _ from 'lodash';
import { computed, ref } from 'vue';
import { useI18n } from 'vue-i18n';

const { t } = useI18n()
const emit = defineEmits(['close'])
const props = defineProps({
  case: String
})

const randomAsic = asicsSheet.filter(el => el.shop)?.[_.random(0, asicsSheet.filter(el => el.shop).length)]?.name?.toUpperCase()
const stars = ref([]);
const starsCount = 20; // кількість зірок
const maxDelay = 5; // сек
const sizeClasses = ["small", "medium", "large"];

const startAnimation = ref(false)
const showPrize = ref(false)
const handleClick = (e) => {
  if(stars.value.length) {
    stars.value = []
    emit('close')
    return
  }
  if (['frame-fadein', 'frame-fadeout', 'case-fadein']?.some(el => e?.animationName?.includes(el))) {
    return
  }
  if (!startAnimation.value) {
    stars.value = []
    startAnimation.value = true
  }
  if (e?.animationName?.includes('case-shaking')) {
    showPrize.value = true
    starsAnimation()
  }
  if (startAnimation.value) {
    return
  }
}

const starsAnimation = () => {
  confetti({
    // shapes: ['star'],
    particleCount: 150,
    spread: 70,
    zIndex: 9999,
    // gravity: 1.5,
    // colors: ['#FCA64380'],
    origin: {
      y: 0.6
    }
  });
  for (let i = 0; i < starsCount; i++) {
    stars.value.push({
      top: `${Math.random() * 100}%`,
      left: `${Math.random() * 100}%`,
      delay: `${(Math.random() * maxDelay).toFixed(2)}s`,
      size: sizeClasses[Math.floor(Math.random() * sizeClasses.length)]
    });
  }
}

const imageCase = () => {
  return computed(() => new URL(`../../assets/cases/${props.case}.webp`, import.meta.url).href);
};

const imagePathAsics = (asic) => {
  const com = computed(
    () => new URL(`../../assets/asics/${asic?.toString().toUpperCase()}.webp`, import.meta.url).href,
  )
  return com
}

// onMounted(() => {
//   setTimeout(() => {
//     handleClick()
//   }, 8000)
// })
</script>

<template>
  <div class="craft-modal-mask" :class="{ unpacking: startAnimation }" @click="handleClick">
    <div class="craft-modal-container">
      <div class="box-frame" :class="[props.case, { unpacking: startAnimation }]" @animationend="handleClick">
        <h1 class="top-title">ASICs BOX</h1>
        <div class="bottom-title">{{ t(`asic_shop.${props.case}`) }}</div>
      </div>
      <div class="shine-overlay">
        <div v-for="(star, index) in stars" :key="index" class="shine-star" :class="star.size" :style="{
          top: star.top,
          left: star.left,
          animationDelay: star.delay
        }"></div>
      </div>
      <div v-if="showPrize" class="win-frame" :class="[props.case]">
        <h1 class="top-title">Ваш выигрыш!</h1>
        <img class="asic-win" :src="imagePathAsics(randomAsic).value" />
        <div class="bottom-title">{{ randomAsic }}</div>
      </div>
      <img class="case" :class="{ unpacking: startAnimation }" :src="imageCase().value" @animationend="handleClick" />
      <div v-if="startAnimation" class="pulsation" :class="props.case"></div>
    </div>
    <div class="get-btn">{{ t('wheel.modal_btn') }}</div>
  </div>
</template>

<style lang="scss" scoped>
$rarity-colors: (
  "legendary": #FCA643,
  "epic": #437EFC,
  "rare": #43FC5F,
  "common": #FFFFFF
);

$rarity-bg-colors: (
  "legendary": #E98509,
  "epic": #437EFC,
  "rare": #009600,
  "common": #B5B5B5
);

.shine-overlay {
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  width: 100%;
  height: 100%;
  pointer-events: none;
  overflow: hidden;
  z-index: 9999;
}

.shine-star {
  position: absolute;
  background-image: url('data:image/png;base64,');
  background-repeat: no-repeat;
  background-position: center;
  background-size: contain;
  opacity: 0;
  animation: glitter 6s linear infinite;
}

.shine-star.small {
  width: 12px;
  height: 12px;
}

.shine-star.medium {
  width: 20px;
  height: 20px;
}

.shine-star.large {
  width: 32px;
  height: 32px;
}

@keyframes glitter {
  0% {
    transform: scale(0.3) rotate(0deg);
    opacity: 0;
  }

  5% {
    transform: scale(1) rotate(360deg);
    opacity: 1;
  }

  50% {
    transform: scale(0.3) rotate(720deg);
    opacity: 0;
  }

  100% {
    transform: scale(0.3) rotate(0deg);
    opacity: 0;
  }
}

.craft-modal-mask {
  position: fixed;
  z-index: 9998;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  background: #00000080;
  backdrop-filter: blur(5px);
  transition: opacity 0.3s ease;

  &.unpacking {
    animation: color-change 6s linear both;

    @keyframes color-change {
      0% {
        bbackground: #00000080;
      }

      100% {
        background: #000000DD;
      }
    }
  }

  .craft-modal-container {
    position: relative;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    text-align: center;
    margin: 0 auto;
    width: 90%;
    height: 50%;
    padding: 15px 0 10px;


    .win-frame {
      position: absolute;
      top: 50%;
      left: 0;
      transform: translateY(-50%);
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: space-between;
      width: 100%;
      height: 90%;
      transition: all 0.3s ease;

      .top-title {
        width: 100%;
        text-align: center;
        font-family: 'Inter', sans-serif;
        font-weight: 600;
        font-size: 28px;
        letter-spacing: 0px;
        color: #fff;
        padding: 10px 0;
      }

      .asic-win {
        position: relative;
        width: 75%;
        z-index: 5;
      }

      @each $rarity, $color in $rarity-bg-colors {
        &.#{$rarity} {
          .bottom-title {
            width: fit-content;
            border-radius: 15px;
            text-align: center;
            font-family: 'Inter', sans-serif;
            font-weight: 600;
            font-size: 25px;
            letter-spacing: 0px;
            color: #fff;
            background: $color;
            padding: 6px 20px;
          }
        }
      }
    }

    .box-frame {
      position: absolute;
      top: 50%;
      left: 0;
      transform: translateY(-50%);
      display: flex;
      flex-direction: column;
      justify-content: space-between;
      width: 100%;
      height: 90%;
      background: #00000060;
      transition: all 0.3s ease;
      border-radius: 20px;
      overflow: hidden;
      animation: frame-fadein 0.6s cubic-bezier(0.230, 1.000, 0.320, 1.000) both;

      &.unpacking {
        animation: frame-fadeout .5s cubic-bezier(0.165, 0.840, 0.440, 1.000) both;
      }

      .top-title {
        width: 100%;
        text-align: center;
        font-family: 'Inter', sans-serif;
        font-weight: 600;
        font-size: 28px;
        letter-spacing: 0px;
        color: #fff;
        padding: 10px 0;
      }

      @each $rarity, $color in $rarity-colors {
        &.#{$rarity} {
          border: 2px solid $color;
          box-shadow: 0px 0px 30px rgba($color, 0.5);

          &::before {
            position: absolute;
            content: "";
            top: 0;
            left: 0;
            width: 100%;
            height: 200%;
            background: radial-gradient(ellipse 40% 60% at center, rgba($color, 0.7), transparent);
            rotate: 45deg;
            transform-origin: 100% 35%;
            background-repeat: no-repeat;
            z-index: 10;
          }

          .bottom-title {
            width: 100%;
            text-align: center;
            font-family: 'Inter', sans-serif;
            font-weight: 600;
            font-size: 28px;
            letter-spacing: 0px;
            color: #fff;
            background: rgba($color, 0.6);
            padding: 6px 0;
          }
        }
      }
    }

    .case {
      position: relative;
      width: 90%;
      z-index: 5;
      animation: case-fadein 0.6s cubic-bezier(0.230, 1.000, 0.320, 1.000) forwards;
      ;

      &.unpacking {
        animation: case-shaking 6s ease-in-out forwards;
      }
    }

    @keyframes animate {
      0% {
        background-position: -500%;
      }

      100% {
        background-position: 500%;
      }
    }

    .pulsation {
      position: absolute;
      content: "";
      top: 50%;
      left: 50%;
      transform: translate(-50%, -50%);
      width: 400px;
      height: 100%;
      animation: glow-pulse 6.9s ease-in-out forwards;
      z-index: 2;

      @each $rarity, $color in $rarity-colors {
        &.#{$rarity} {
          background: radial-gradient(ellipse 40% 40% at center, $color, transparent);
        }
      }
    }
  }

  .get-btn {
    margin-top: 50px;
    font-family: 'Inter';
    font-weight: 600;
    font-size: 28px;
    overflow: hidden;
    background: linear-gradient(90deg, transparent, #fff, transparent);
    background-repeat: no-repeat;
    background-size: 80%;
    z-index: 100;
    animation: animate 3s linear infinite;
    -webkit-background-clip: text;
    -webkit-text-fill-color: rgba(255, 255, 255, 0);
  }

}

@keyframes glow-pulse {
  0% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  5% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  10% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  15% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  20% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  25% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  30% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  35% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  40% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  45% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  49% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  53% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  57% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  61% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  64% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  67% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  70% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  73% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  75% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  77% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  79% {
    transform: translate(-50%, -50%) scale(1);
    opacity: 0.6;
  }

  81% {
    transform: translate(-50%, -50%) scale(1.3);
    opacity: 1;
  }

  85% {
    transform: translate(-50%, -50%) scale(2.2);
    opacity: 0.5;
  }

  93% {
    transform: translate(-50%, -50%) scale(3.5);
    opacity: 0.2;
  }

  100% {
    transform: translate(-50%, -50%) scale(5);
    opacity: 0;
  }
}

@keyframes case-shaking1 {
  0% {
    transform: scale(1) translate(0, 0) rotate(0deg);
  }

  4% {
    transform: scale(1.2) translate(5px, -30px) rotate(3deg);
  }

  8% {
    transform: scale(1.3) translate(5px, 5px) rotate(3deg);
  }

  12% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-3deg);
  }

  16% {
    transform: scale(1.3) translate(5px, 5px) rotate(3deg);
  }

  20% {
    transform: scale(1.3) translate(-25px, 5px) rotate(-3deg);
  }

  24% {
    transform: scale(1.3) translate(15px, 25px) rotate(5deg);
  }

  28% {
    transform: scale(1.3) translate(-25px, 15px) rotate(-5deg);
  }

  32% {
    transform: scale(1.3) translate(15px, 25px) rotate(5deg);
  }

  36% {
    transform: scale(1.3) translate(-15px, 5px) rotate(-5deg);
  }

  40% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  44% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  48% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  52% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  56% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  60% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  64% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  68% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  72% {
    transform: scale(1.2) translate(5px, 5px) rotate(5deg);
  }

  76% {
    transform: scale(1.2) translate(-5px, 5px) rotate(-5deg);
  }

  80% {
    transform: scale(1.1) translate(5px, 5px) rotate(5deg);
  }

  84% {
    transform: scale(1.1) translate(-5px, 5px) rotate(-5deg);
  }

  88% {
    transform: scale(1.0) translate(5px, 5px) rotate(5deg);
  }

  90% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  92% {
    transform: scale(1.5) translate(5px, 5px) rotate(5deg);
    filter: blur(0px);
  }

  94% {
    transform: scale(2.8) translate(-5px, -5px) rotate(-5deg);
    filter: blur(1px);
    opacity: 1;
  }

  96% {
    transform: scale(5.5) translate(5px, 5px) rotate(5deg);
    filter: blur(2px);
    opacity: 0.8;
  }

  98% {
    transform: scale(8.5) translate(-5px, -5px) rotate(-5deg);
    filter: blur(3px);
    opacity: 0.5;
  }

  100% {
    transform: scale(10) translate(5px, 5px) rotate(5deg);
    filter: blur(5px);
    opacity: 0;
  }
}

@keyframes case-fadein {
  0% {
    -webkit-transform: translateX(-1000px) scaleX(2.5) scaleY(0.2);
    transform: translateX(-1000px) scaleX(2.5) scaleY(0.2);
    -webkit-transform-origin: 100% 50%;
    transform-origin: 100% 50%;
    -webkit-filter: blur(40px);
    filter: blur(40px);
    opacity: 0;
  }

  100% {
    -webkit-transform: translateX(0) scaleY(1) scaleX(1);
    transform: translateX(0) scaleY(1) scaleX(1);
    -webkit-transform-origin: 50% 50%;
    transform-origin: 50% 50%;
    -webkit-filter: blur(0);
    filter: blur(0);
    opacity: 1;
  }
}

@keyframes frame-fadein {
  0% {
    -webkit-transform: translateX(-1000px) translateY(-50%) scaleX(2.5) scaleY(0.2);
    transform: translateX(-1000px) translateY(-50%) scaleX(2.5) scaleY(0.2);
    -webkit-transform-origin: 100% 50%;
    transform-origin: 100% 50%;
    -webkit-filter: blur(40px);
    filter: blur(40px);
    opacity: 0;
  }

  100% {
    -webkit-transform: translateX(0) translateY(-50%) scaleY(1) scaleX(1);
    transform: translateX(0) translateY(-50%) scaleY(1) scaleX(1);
    -webkit-transform-origin: 50% 50%;
    transform-origin: 50% 50%;
    -webkit-filter: blur(0);
    filter: blur(0);
    opacity: 1;
  }
}

@keyframes frame-fadeout {
  0% {
    -webkit-transform: translateY(-50%) scale(1);
    transform: translateY(-50%) scale(1);
    -webkit-filter: blur(0px);
    filter: blur(0px);
    opacity: 1;
  }

  100% {
    -webkit-transform: translateY(-50%) scale(2);
    transform: translateY(-50%) scale(2);
    -webkit-filter: blur(10px);
    filter: blur(10px);
    opacity: 0;
  }
}



@keyframes case-shaking {
  0% {
    transform: scale(1) translate(0, 0) rotate(0deg);
  }

  4% {
    transform: scale(1.2) translate(5px, -30px) rotate(3deg);
  }

  8% {
    transform: scale(1.3) translate(5px, 5px) rotate(3deg);
  }

  12% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-3deg);
  }

  19% {
    transform: scale(1.3) translate(5px, 5px) rotate(3deg);
  }

  26% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-3deg);
  }

  33% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  40% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  45% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  50% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  55% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  60% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  63% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  66% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  69% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  72% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  75% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  77% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  79% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  81% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  83% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  85% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  87% {
    transform: scale(1.3) translate(5px, 5px) rotate(5deg);
  }

  89% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  90% {
    transform: scale(1.2) translate(5px, 5px) rotate(5deg);
  }

  91% {
    transform: scale(1.2) translate(-5px, 5px) rotate(-5deg);
  }

  92% {
    transform: scale(1.2) translate(5px, 5px) rotate(5deg);
  }

  93% {
    transform: scale(1.2) translate(-5px, 5px) rotate(-5deg);
  }

  94% {
    transform: scale(1.1) translate(5px, 5px) rotate(5deg);
  }

  94.5% {
    transform: scale(1.1) translate(-5px, 5px) rotate(-5deg);
  }

  95% {
    transform: scale(1.1) translate(5px, 5px) rotate(5deg);
  }

  95.5% {
    transform: scale(1.1) translate(-5px, 5px) rotate(-5deg);
  }

  96% {
    transform: scale(1.0) translate(5px, 5px) rotate(5deg);
  }

  96.5% {
    transform: scale(1.3) translate(-5px, 5px) rotate(-5deg);
  }

  97% {
    transform: scale(1.5) translate(5px, 5px) rotate(5deg);
    filter: blur(0px);
  }

  97.5% {
    transform: scale(2.8) translate(-5px, -5px) rotate(-5deg);
    filter: blur(1px);
    opacity: 1;
  }

  98% {
    transform: scale(5.5) translate(5px, 5px) rotate(5deg);
    filter: blur(2px);
    opacity: 0.8;
  }

  99% {
    transform: scale(8.5) translate(-5px, -5px) rotate(-5deg);
    filter: blur(3px);
    opacity: 0.5;
  }

  100% {
    transform: scale(10) translate(5, 5px) rotate(5deg);
    filter: blur(5px);
    opacity: 0;
  }
}
</style>