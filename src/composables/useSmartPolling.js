...
export function useSmartPolling(url, options = {}) {
  const {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      controller = new AbortController() // Створюємо новий контролер
        ...
        const storedLastModified = localStorage.getItem('updDate')
      ...
      const response = await host.get(url, { signal: controller.signal })
