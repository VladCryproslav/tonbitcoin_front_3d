...
# Lottery Views
class LotteryDataView(APIView):
    @swagger_auto_schema(
        tags=["lottery"],
        operation_description="Get lottery data",
        responses={
            200: LotterySerializer,
        },
    )
    def get(self, request):
        try:
...
class LotteryParticipantsView(APIView):
    @swagger_auto_schema(
        tags=["lottery"],
        operation_description="Get lottery participants",
        responses={
            200: LotteryParticipantSerializer(many=True),
        },
    )
    def get(self, request):
        try:
...
class BuyLotteryTicketView(APIView):
    @swagger_auto_schema(
        tags=["lottery"],
        operation_description="Buy lottery ticket",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                'wallet_address': openapi.Schema(type=openapi.TYPE_STRING),
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if amount != lottery.ticket_price:
                    return Response({'error': f'Invalid amount. Expected {lottery.ticket_price} TON'}, status=status.HTTP_400_BAD_REQUEST)
                ...
                print(f"DEBUG: User username: {user_profile.username if user_profile else 'No user'}")
